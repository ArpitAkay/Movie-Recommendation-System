from sklearn.feature extraction.text Import ffidfvectorizer
...
print(c)
m= df2[ 'vote_count'].quantile(0.9) movies having vote count greater than
print(m)
lists_movies = df2.copy().loc[df2[ 'vote_count']]
lists movies.shape
...
def movieteller(title):
    ...
    for i in range(0, len(movie_indices)):
        for j in range(0, len(df2));
            if movie_indices[i]==df2.iloc[j]["no"]:
                li.append(df2.iloc[j]["original_title"])
                11.append(df2.iloc[j]["overview"])
                break
    return li,l1
def popular_movies():
        l3=[]
